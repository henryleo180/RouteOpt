...
namespace RouteOpt::Application::CVRP {
    namespace TestingDetail {
        inline void addBranchConstraint(std::vector<int> &cind, std::vector<double> &cval, Solver &node_solver,
                                        bool dir = false) {
            if (dir) {
                if (cind.front() == 0) {
                    cval.front() = 1;
        ...
        void callLabelingInTesting(CVRPSolver *cvrp, BbNode *node) {
            ...
            cvrp->solveLPInLabeling(node, if_open_heur, if_open_exact, if_update_node_val,
                                    if_consider_regenerate_bucket_graph, if_possible_terminate_early, if_fix_row,
                                    if_fix_meet_point, if_allow_delete_col, labeling_time_limit);
    ...
    void CVRPSolver::processCGTesting(BbNode *node, const std::pair<int, int> &edge, double &dif1, double &dif2) {
        ...
        if (node->getIfInEnumState())
            node->addBranchConstraint2ColPoolInEnumByColMap(edge, pricing_controller.getColumnPoolPtr());
        ...
        if (node->getIfInEnumState())
            node->refMatrixColPool().pop_back();
    }


    template<bool if_exact>
    void CVRPSolver::processOneSideCGTesting(BbNode *node, const std::pair<int, int> &edge, double &dif1, bool dir) {
        auto &node_solver = node->refSolver();
    ...
    inline void BbNode::obtainBrcMap() {
        if (!edge_col_map.empty())
            return;
        // std::unordered_map<std::pair<int, int>, std::vector<std::pair<int, int> > > edge_col_map{}; //edge, col idx,
        // cnt
        ...
        if (ai) {
            ...
            for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(mat0, aj - 1); it; ++it) {
                if (it.value() > 0.5) {
                    ...
                    for (;; ++j) {
                        if (int current_node = col_pool4_pricing[j]; !current_node)
                            break;
                    }
    ...
    inline void BbNode::obtainColIdxNotAllowByEdge(const std::pair<int, int> &edge, std::vector<int> &col_idx) {
        ...
        int nz;
        if (ai != 0) {
            SAFE_SOLVER(solver.getConstraints(&nz, beg.data(), ind.data(), val.data(), ai - 1, 1))
            st_.insert(ind.begin(), ind.begin() + nz);
        }
        SAFE_SOLVER(solver.getConstraints(&nz, beg.data(), ind.data(), val.data(), aj - 1, 1))
        st_.insert(ind.begin(), ind.begin() + nz);
        for (auto &col: map_[edge]) {
            st_.erase(col.first);
        }
    ...
    inline std::unordered_map<std::pair<int, int>, double, PairHasher> BbNode::obtainSolEdgeMap(BbNode *node) {
        ...
        for (int i = 0; i < cols.size(); ++i) {
            ...
            if (seq.size() != 1)
                edge_map[{0, b4}] += val; // if single point, just be one count;
        }
        // check the edge

        PRINT_REMIND("disable 1 approx in call branching for three branching;p");
