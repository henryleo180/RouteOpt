...
namespace RouteOpt::Application::CVRP
{
    namespace CuttingDetail
    {
        ...
        inline bool increaseCondition2(double ub, double lb, double gap_improved, int &tail_off_counter)
        {
            ...
            if ((gap_improved / (ub - lb)) < RELATIVE_IMPROVE_THRESHOLD)
            {
                ++tail_off_counter;
                if constexpr (if_print_tail_reason)
                    PRINT_REMIND("2: gap improved= " + std::to_string(gap_improved) + " ratio= " + std::to_string(gap_improved / (ub - lb)) + " threshold= " + std::to_string(RELATIVE_IMPROVE_THRESHOLD));
        ...
        inline bool increaseCondition3(double lb, double gap_improved, int &tail_off_counter)
        {
            if ((gap_improved / lb) < LB_RELATIVE_IMPROVE_THRESHOLD)
            {
                ++tail_off_counter;
                if constexpr (if_print_tail_reason)
                    PRINT_REMIND("3: gap improved= " + std::to_string(gap_improved) + " ratio= " + std::to_string(gap_improved / lb) + " threshold= " + std::to_string(LB_RELATIVE_IMPROVE_THRESHOLD));
                return true;
            }
            return false;
        }
                                 ...
                                 int &tail_off_counter)
        {
            ...
            {
                if (equalFloat(eps_max, 0.))
                    eps_max = std::numeric_limits<float>::max();
                if (eps > CUTTING_BRANCHING_RATIO * gap_improved / br_value_improved * eps_max)
                {
                    if_tail_off = true;
                    if constexpr (if_print_tail_reason)
                        PRINT_REMIND("eps= " + std::to_string(eps) + " threshold= " + std::to_string(CUTTING_BRANCHING_RATIO * gap_improved / br_value_improved * eps_max));
                ...
                if (gap_improved < CUTTING_BRANCHING_RATIO * br_value_improved)
                {
                    if_tail_off = true;
                    if constexpr (if_print_tail_reason)
                        PRINT_REMIND("gap improved= " + std::to_string(gap_improved) + " threshold= " + std::to_string(CUTTING_BRANCHING_RATIO * br_value_improved));
                    goto QUIT;
                }
                if (eps > TailOffHardTime)
                {
                    if_tail_off = true;
                    if constexpr (if_print_tail_reason)
                        PRINT_REMIND("eps= " + std::to_string(eps) + " tail off hard time= " + std::to_string(TailOffHardTime));
        ...
        QUIT:
            if (eps_max == std::numeric_limits<float>::max())
                eps_max = 0.;
            ...
            Solver &solver)
        {
            ...
            if (if_elementary)
                std::fill(sense.begin(), sense.end(), SOLVER_GREATER_EQUAL);
    ...
    CUTTING:
        ...
        if (!CuttingDetail::if_pure_rcc_tail)
            goto PRICING;

        // Only disable R1C in the child node
        PRINT_REMIND("Only disable R1C in the child node")
    ...
    PRICING:
        ...
        if (node->getIfInEnumState())
            node->changeEnumMatByCuts(rank1_coefficient_getter);
    ...
    ENTER:
        ...
        if (node->getIfInEnumState() != old_enu_state)
            tail_off_counter = 0;
