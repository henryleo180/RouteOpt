...
namespace RouteOpt::Application::CVRP
{
    ...
    {
        delete[] all_label;
        // instead of doubling blindly, increment by a safe step:
        size_t next = std::min(label_assign + 2'000'000, label_assign * 2);
        if (next == label_assign)
            THROW_RUNTIME_ERROR("Cannot increase labels beyond " + std::to_string(label_assign));
        label_assign = next;
        try
        {
            all_label = new Label[label_assign];
        ...
        {
            THROW_RUNTIME_ERROR("Failed to allocate " + std::to_string(label_assign) + " labels.");
        }
    }
    ...
    inline void cleanNegativeTuple(std::vector<std::tuple<Label *, Label *, double>> &negative_rc_label_tuple,
                                   int num)
    {
        if (negative_rc_label_tuple.empty())
            return;
    ...
    inline void CVRP_Pricing::setTerminateMarker(double val, double ub, bool &if_terminate)
    {
        ...
        {
            if (ceilTransformedNumberRelated(smallest_rc * max_num_vehicle_ref + val + RC_TOLERANCE) + TOLERANCE >= ub)
            {
                if_terminate = true;
            }
        }
