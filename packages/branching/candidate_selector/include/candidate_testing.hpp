...
namespace RouteOpt::Branching::CandidateSelector
{
    namespace CandidateSelectorDetail
    {
        ...
        void printScore(const std::vector<CandidateScoreInfo<BrCType>> &edge_info)
        {
            ...
            {
                const auto &edge = edge_info[i];
                std::ostringstream oss;

                // handle pair<int,int> vs vector<pair<int,int>>
                if constexpr (std::is_same_v<BrCType, std::vector<std::pair<int, int>>>)
                {
                    // multiple edges in BrCType
                    for (size_t j = 0; j < edge.brc.size(); ++j)
                    {
                        oss << std::setw(3) << std::right << edge.brc[j].first
                            << "-"
                            << std::setw(3) << std::left << edge.brc[j].second;
                        if (j + 1 < edge.brc.size())
                            oss << ",";
                    }
                }
                else
        ...
        TestingPhase phase)
    {
        ...
        if (branch_pair.size() <= 1)
        {
            CANDIDATE_SELECTOR_VERBOSE_EXEC(
                PRINT_REMIND("no " + marker + " testing is needed: too few candidates: " + std::to_string(branch_pair.size())))
            return;
        }
